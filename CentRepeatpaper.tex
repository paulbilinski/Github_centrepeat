...
\newcommand{\pb}[1]{\todo[size=\scriptsize, color=brown]{#1}}
...
\\
\bigskip
\jri{please add Matt's affiliation}
...
Here, we utilize sequence assembly and read mapping to identify and quantify the genomic abundance of between different tandem repeat sequences.
...
\cite{melters2013comparative}\jri{can't figure out how to make it put name first. natbib does this easily with citet, but we can't use that here??} applied their approach broadly across more than 280 plant and animal taxa, relying on several model organisms to serve as anchors to reaffirm several findings.\jri{this last clause is weird}
They assume that the most abundant tandem repeat in all taxa is the centromere repeat, and use this assumption to study centromere repeat evolution broadly.\jri{repetitive}
While the approach appears to work well in cases for which a reference genome is assembled and the organisms is animal, the method has known problems when working with plants.\jri{awkwards sentence}
...
Maize and sorghum are ideal anchor species as both have reference genomes and their centromere repeats are diverged from one another.\citex
Because previous work has shown that sequencing libraries prepared through a identical methods retain relative composition of repetitive content (CITE)\jri{wha? i don't think anyone has shown this. instead they've shown the converse}, we elect to re-sequence species used in the study.
...
We show that the assumption of Melters et al\jri{fix as proper cite} that the highest abundance tandem repeat is a centromere is not supported in grasses, especially when the centromere repeat is novel or unknown.
...
The ability to identify novel repeats from low cost sequencing can enable a new type of comparative genomic study that tracks the evolution of genomic composition.\jri{not sure I buy this.your paper suggests method don't work well!}
...
The topologies represent approximate relationships that were more thoroughly investigated in \cite{wu2012phylogeny}.\jri{do they agree? is wu 2012 part of wu-tang?}
...
\label{phylotree}
\end{figure}
\jri{is that a cladogram or an NJ tree?}
...
We ran Tandem Repeat Finder \cite{benson1999tandem}  (TRF, ; parameters: 2 7 7 80 10 50 2000 -h) on all assembled contigs to select only those that contained tandem repeats.  \jri{params need to go in separate sentence. include formal citations} 
...
To discover the genomic composition of each tandemly repeated contig, we used Mosaik \citex, which stores information about multiply mapping reads (version 1.0; parameters optimized for tandem repetitive elements as in \cite{bilinski2014diversity}.
...
Scripts and step by step guides are available on \url{https://github.com/paulbilinski/Github_centrepeat}. \jri{kudos for githubbin'}
...
\label{totaltrf}
\end{figure}
\jri{pretty, but i would prefer to see them ranked in descending order, with taxa in same order on side, with no fancy colors. black or grey bars on white background. this is fancier than is necessary to usefully convey info. try theme bw}
...
The number of post-TRF contigs \jri{not sure what post-TRF means? these are same contigs referred to in previous sentence?} varied between taxa based on coverage and overall genomic repetitive content.
...
Using the Kew C-Value database and literature (what lit do we cite?) \jri{see \url{http://data.kew.org/cvalues/CvalServlet?querytype=1}}, we obtained point estimates for genome size in each of our taxa.
...
Several other taxa (LIST)\jri{yes please include list!} had top ranked contigs that shared homology with the \emph{Sorghum} centromere repeat, suggesting that the method is robust in these taxa as well.
However, our analyses also yielded several taxa whose top ranked contig had no homology to known centromere repeats.
We chose to test the robustness of the Melters et al (2013)\jri{formal cite} assumption in these taxa with fluorescent in-situ hybridization (FISH).
In \emph{Urelytrum} and \emph{Hyparrhenia}, FISH results did not suggest that the highest ranked contig was centromeric, as probe illumination was scattered across chromosomes, not often found near central locations, or confined to only some of the chromosomes \ref{FISH}.\jri{for clarity, i often do fig:FISH for my labels, makes it easier reading later on}
...
(do i want to go into detail about how those group up and whether they are centromeric here? Maybe a figure of homology between each ranking?) \jri{i like the idea of a figure. FISH and homology results should be in results not here.}
...
The ability to look broadly across a phylogeny at consensus repeats may provide a glimpse into the ancestral condition of tandem repeats of andropogonaeae; tandem repeats that gave rise to the structurally important centromere repeats of maize and sorghum.\jri{this is a bold claim. any evidence?}
While far from sufficient to make evolutionary claims about the origin of functionally important tandem repeats \jri{wait you just did}, the identification of novel repeats in previously unstudied organisms has the potential to bring a large volume of data to comparative genomics.\jri{lame sentence. bring a lotta data? big deal.}
